...
def circularise(at_apoapsis = True, rcs = False): # Circularises (default at periapsis, no RCS)
	...
	delta_v = v2 - v1
	node = vessel.control.add_node(ut() + (vessel.orbit.time_to_apoapsis if at_apoapsis else vessel.orbit.time_to_periapsis), prograde=delta_v)
	...
	burn_ut = ut() + (vessel.orbit.time_to_apoapsis if at_apoapsis else vessel.orbit.time_to_periapsis) - (burn_time / 2.)
	lead_time = 5
...
def set_altitude(desired_alt, at_apoapsis = True, rcs = False):
	print("Planning burn...")
	delta_v = hohmann_elliptical((vessel.orbit.apoapsis if at_apoapsis else vessel.orbit.periapsis), desired_alt + vessel.orbit.body.equatorial_radius)
